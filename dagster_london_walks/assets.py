...
def write_transformation_to_s3(file_from_s3, s3: S3Resource) -> MaterializeResult:
    ...
    return MaterializeResult(
        metadata={
            "Total Length in Miles": MetadataValue.float(float(round(data_with_km.distance_miles.sum(), 2)),
            "Total Length in Kilometers": MetadataValue.float(float(round(data_with_km.distance_km.sum(), 2))
        }
    )
